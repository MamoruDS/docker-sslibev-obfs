...
ENV EXEC ${EXEC:-'ss-server'}
ENV ARGS ''
ENV OBFS_OPT ${OBFS_OPT:-'--plugin obfs-server --plugin-opts "obfs=http"'}
COPY --from=build-env /usr/bin/ss-* /usr/bin/
COPY --from=build-env /usr/local/bin/obfs-* /usr/local/bin/
...
# CMD ss-server -c config.json ${ARGS} ${obfs:+'--plugin obfs-server --plugin-opts "obfs=http"'}
CMD ${EXEC} -c config.json ${ARGS} ${OBFS_OPT}